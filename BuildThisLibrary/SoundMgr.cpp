...
bool SoundMgr::PlayFmodSound(string _instanceName)
{
	FMOD_RESULT fr = fmod_System->playSound(soundList.find(_instanceName.c_str())->second, nullptr, false, &fmod_Channel);
	if (fr != FMOD_OK)
	{
...
{
	float getVolume = 0;
	fmod_Channel->getVolume(&getVolume);

	if (getVolume > 0.1f)
	{
		getVolume -= 0.1f;
		fmod_Channel->setVolume(getVolume);
	}
	if (getVolume <= 0.1f)
		soundList.find(_instanceName.c_str())->second->release();
}
